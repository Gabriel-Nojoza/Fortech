...
type CustomChatMeasure = DatasetMeasure & {
  dependencies?: {
...
const CUSTOM_CHAT_MEASURES: CustomChatMeasure[] = [
  {
    tableName: "META",
    measureName: "Meta",
    ...
    isHidden: false,
    dependencies: {
      columns: [{ tableName: "META", columnName: "VLVENDAPREV" }],
      measures: ["Check"],
    },
  },
  {
...
type ExtractedWebhookChartPayload = {
  chartType: "bar" | "line" | "pie" | null
  data: StructuredChartDataResult | null
}
...
function normalizeWebhookChartType(value: unknown): "bar" | "line" | "pie" | null {
  const normalized = String(value ?? "")
    .trim()
    .toLowerCase()

  if (!normalized) return null
  if (normalized.includes("pie") || normalized.includes("pizza") || normalized.includes("torta")) {
    return "pie"
  }
  if (normalized.includes("line") || normalized.includes("linha")) {
    return "line"
  }
  if (normalized.includes("bar") || normalized.includes("barra") || normalized.includes("coluna")) {
    return "bar"
  }
...
function buildStructuredChartResultFromChartBlock(value: unknown): ExtractedWebhookChartPayload | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return null
  }

  const record = value as Record<string, unknown>
  const chartType = normalizeWebhookChartType(record.chartType)
  const labels = Array.isArray(record.labels)
    ? record.labels.map((label) => String(label ?? "").trim())
    : []
  const datasets = Array.isArray(record.datasets)
    ? record.datasets
        .map((dataset) => {
          ...
          return data.length > 0 ? { label, data } : null
        })
        .filter((dataset): dataset is { label: string; data: Array<number | null> } => Boolean(dataset))
    : []

  if (!chartType || labels.length === 0 || datasets.length === 0) {
    return null
  }

  const labelColumnName = "Categoria"
  const valueColumns = datasets.map((dataset) => ({
    name: dataset.label,
    dataType: "Double",
  }))
  ...
  return {
    ...
    data: {
      columns: [
        { name: labelColumnName, dataType: "String" },
        ...valueColumns,
      ],
      rows,
    },
...
function extractChartPayloadFromUnknown(value: unknown): ExtractedWebhookChartPayload | null {
  const directTable = extractStructuredChartResultFromUnknown(value)
  if (directTable) {
    return {
      chartType: null,
      data: directTable,
  ...
  if (typeof value === "string") {
    ...
    if (directJson) {
      try {
        return extractChartPayloadFromUnknown(JSON.parse(directJson))
      } catch {
        // ignore invalid JSON
      }
    }
    ...
    if (fenced?.[1]) {
      try {
        return extractChartPayloadFromUnknown(JSON.parse(fenced[1]))
      } catch {
        // ignore invalid fenced JSON
      }
    }
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      const nested = extractChartPayloadFromUnknown(item)
      if (nested) return nested
    }
    return null
  }

  const chartBlock = buildStructuredChartResultFromChartBlock(value)
  if (chartBlock) {
    return chartBlock
  }
  ...
  for (const key of candidateKeys) {
    const nested = extractChartPayloadFromUnknown(record[key])
    if (nested) return nested
  }

  for (const nestedValue of Object.values(record)) {
    const nested = extractChartPayloadFromUnknown(nestedValue)
    if (nested) return nested
  }
...
export function extractWebhookChartPayload(payload: unknown): ExtractedWebhookChartPayload {
  return (
    extractChartPayloadFromUnknown(payload) ?? {
      chartType: null,
      data: null,
    }
  )
...
function getColumnKey(tableName: string, columnName: string) {
  return `${normalizeObjectName(tableName)}::${normalizeObjectName(columnName)}`
}

const CUSTOM_CHAT_MEASURE_DEPENDENCIES = new Map<
  string,
  NonNullable<CustomChatMeasure["dependencies"]>
>([
  [
    normalizeMeasureName(CUSTOM_CHAT_MEASURES[0]?.measureName ?? "Meta"),
    {
      columns: [{ tableName: "META", columnName: "VLVENDAPREV" }],
      measures: ["Check"],
    },
  ...
  [
    normalizeMeasureName(CUSTOM_CHAT_MEASURES[1]?.measureName ?? "Vl Devolucao"),
    {
      measures: ["Vl Devolucao Venda", "Vl Devolucao Avulsa"],
    },
  ],
  [
    normalizeMeasureName(CUSTOM_CHAT_MEASURES[2]?.measureName ?? "Pedidos Enviados"),
    {
      columns: [
        { tableName: "PEDIDOS", columnName: "QT" },
        { tableName: "PEDIDOS", columnName: "PRUNIT" },
      ],
      measures: ["Check"],
    },
  ],
  [
    normalizeMeasureName(CUSTOM_CHAT_MEASURES[3]?.measureName ?? "Pedidos Enviados - Dev."),
    {
      measures: [
        CUSTOM_CHAT_MEASURES[2]?.measureName ?? "Pedidos Enviados",
        CUSTOM_CHAT_MEASURES[1]?.measureName ?? "Vl Devolucao",
      ],
    },
  ],
  [
    normalizeMeasureName(CUSTOM_CHAT_MEASURES[4]?.measureName ?? "$ Tendencia Ped."),
    {
      measures: [
        CUSTOM_CHAT_MEASURES[3]?.measureName ?? "Pedidos Enviados - Dev.",
        "Dias Realizados",
        "Dias Uteis",
      ],
    },
  ...
  [
    normalizeMeasureName(CUSTOM_CHAT_MEASURES[5]?.measureName ?? "% Tendencia Ped."),
    {
      measures: [
        CUSTOM_CHAT_MEASURES[4]?.measureName ?? "$ Tendencia Ped.",
        CUSTOM_CHAT_MEASURES[0]?.measureName ?? "Meta",
      ],
    },
  ],
])

function getCustomMeasureDependencies(measureName: string) {
  return CUSTOM_CHAT_MEASURE_DEPENDENCIES.get(normalizeMeasureName(measureName))
}

function canUseCustomMeasure(
  measure: CustomChatMeasure,
  availableColumns: Set<string>,
  availableMeasureNames: Set<string>
) {
  const dependencies = measure.dependencies ?? getCustomMeasureDependencies(measure.measureName)
  const requiredColumns = dependencies?.columns ?? []
  const requiredMeasures = dependencies?.measures ?? []

  const hasColumns = requiredColumns.every((column) =>
    availableColumns.has(getColumnKey(column.tableName, column.columnName))
  )
  ...
  return requiredMeasures.every((measureName) =>
    availableMeasureNames.has(normalizeMeasureName(measureName))
  )
}

export function getAvailableCustomChatMeasures(
  metadata: DatasetMetadata
): CustomChatMeasure[] {
  const availableColumns = new Set(
    metadata.columns.map((column) => getColumnKey(column.tableName, column.columnName))
  )
  const availableMeasureNames = new Set(
    metadata.measures.map((measure) => normalizeMeasureName(measure.measureName))
  )
  const remainingMeasures = [...CUSTOM_CHAT_MEASURES]
  const availableCustomMeasureNames = new Set<string>()

  let progress = true
  while (progress) {
    progress = false

    for (let index = remainingMeasures.length - 1; index >= 0; index -= 1) {
      const measure = remainingMeasures[index]
      if (!canUseCustomMeasure(measure, availableColumns, availableMeasureNames)) {
        continue
      }

      const normalizedName = normalizeMeasureName(measure.measureName)
      availableCustomMeasureNames.add(normalizedName)
      availableMeasureNames.add(normalizedName)
      remainingMeasures.splice(index, 1)
      progress = true
    }
  }

  return CUSTOM_CHAT_MEASURES.filter((measure) =>
    availableCustomMeasureNames.has(normalizeMeasureName(measure.measureName))
  )
}
...
export function withCustomChatMeasures(metadata: DatasetMetadata): DatasetMetadata {
  ...
  const appendedMeasures = getAvailableCustomChatMeasures(metadata)
    .filter((measure) => !existingMeasureNames.has(normalizeMeasureName(measure.measureName)))
    .map((measure) => ({
      ...measure,
...
function getReferencedCustomMeasures(
  daxQuery: string,
  availableCustomMeasures: CustomChatMeasure[]
) {
  const normalized = daxQuery.toLowerCase()
  const availableCustomMeasureMap = new Map(
    availableCustomMeasures.map((measure) => [
      normalizeMeasureName(measure.measureName),
      measure,
    ])
  )
  const referencedMeasureNames = new Set<string>()
  const queue = availableCustomMeasures.filter((measure) =>
  ...
  while (queue.length > 0) {
    ...
    const normalizedName = normalizeMeasureName(measure.measureName)
    if (referencedMeasureNames.has(normalizedName)) {
      continue
    }

    referencedMeasureNames.add(normalizedName)

    for (const dependencyName of getCustomMeasureDependencies(measure.measureName)?.measures ??
      []) {
      const dependency = availableCustomMeasureMap.get(normalizeMeasureName(dependencyName))
      if (dependency) {
  ...
  return availableCustomMeasures.filter((measure) =>
    referencedMeasureNames.has(normalizeMeasureName(measure.measureName))
  )
}

export function injectCustomMeasuresIntoDax(
  daxQuery: string,
  metadata?: DatasetMetadata
) {
  const existingMeasureNames = new Set(
  ...
  const referencedMeasures = getReferencedCustomMeasures(
    daxQuery,
    metadata
      ? getAvailableCustomChatMeasures(metadata).filter(
          (measure) =>
            !existingMeasureNames.has(normalizeMeasureName(measure.measureName))
        )
      : CUSTOM_CHAT_MEASURES
  )
  ...
  const measureDefinitions = referencedMeasures.map(
    (measure) =>
